...
static int
append_chars(b,s,n)struct dynamic_buffer*b;const char*s;int n;
{if(!n)return 0;
 if(!b->allocated)
...
append_char(b,c)struct dynamic_buffer*b;int c;
{char _=c;return append_chars(b,&_,1);}
static void
clear_buffer(b)struct dynamic_buffer*b;{b->used=0;}
static void
free_buffer(b)struct dynamic_buffer*b;
{if(b->allocated){free(b->s);b->allocated=0;}}
static void
show_buff(b)const struct dynamic_buffer*b;
{int i;putc('`',stderr);
